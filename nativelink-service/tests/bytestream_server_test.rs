...
/// Companion to `active_uuid_collision_is_rejected_as_retriable`, guarding the
/// *liveness* half of the same bug rather than the correctness half.
///
/// Two Write RPCs share a UUID while the first is mid-stream (`Entry::Occupied`
/// + `idle_stream == None` — Case 3 in `create_or_join_upload_stream`). The
/// original defect re-acquired `active_uploads` while already holding it, which
/// self-deadlocks on parking_lot's non-reentrant `Mutex`. Our fix decides the
/// whole case under a single guard and returns `Code::Aborted`, so the second
/// RPC must come back promptly with that status instead of hanging.
///
/// The `std::thread` watchdog is what makes this a real deadlock test:
/// `tokio::time::timeout` does not reliably fire when a runtime worker is parked
/// on a sync mutex, so an in-runtime timeout would hang with the server rather
/// than fail the test.
#[nativelink_test]
async fn uuid_collision_does_not_deadlock() -> Result<(), Box<dyn core::error::Error>> {
    ...
    // Write 2: same UUID while write 1 is active — triggers Case 3. It must
    // RETURN (not hang) and it must be retriable.
    ...
    let status = join2
        ...
        .expect_err("Active-UUID collision should be rejected, not accepted");
    assert_eq!(
        status.code(),
        tonic::Code::Aborted,
        "Collision must be retriable (Aborted), got: {status:?}"
