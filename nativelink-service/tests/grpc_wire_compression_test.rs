...
async fn small_blob_uses_identity_path() -> Result<(), Box<dyn core::error::Error>> {
    ...
    // The identity upload path derives the resource_name's digest_function
    // segment from the DigestHasherFunc in the OTel context and refuses the
    // upload when it is absent, rather than silently defaulting. `make_content`
    // digests with sha256, so declare that. (The compressed path never reaches
    // this, which is why only the identity-path tests need it.)
    let ctx_guard = make_ctx_for_hash_func(DigestHasherFunc::Sha256)?.attach();
    grpc_store.update_oneshot(digest, content.clone()).await?;
    let fetched = grpc_store.get_part_unchunked(digest, 0, None).await?;
    drop(ctx_guard);
...
async fn flag_off_plain_round_trip() -> Result<(), Box<dyn core::error::Error>> {
    ...
    // Compression is off, so this takes the identity upload path, which
    // requires the client's DigestHasherFunc in the OTel context (see
    // `small_blob_uses_identity_path`). `make_content` digests with sha256.
    let ctx_guard = make_ctx_for_hash_func(DigestHasherFunc::Sha256)?.attach();
    grpc_store.update_oneshot(digest, content.clone()).await?;
    let fetched = grpc_store.get_part_unchunked(digest, 0, None).await?;
    drop(ctx_guard);
