...
use nativelink_config::stores::{
    ExistenceCacheSpec, RedisMode, RedisSearchBackend, RedisSpec, StoreSpec,
};
...
fn test_search_by_index_failure() -> Result<(), Error> {
    ...
    assert!(logs_contain(
        "Error calling ft.aggregate e=TEST - Client: unexpected command index=\"test:_content_prefix_sort_key_3e762c15\" query=\"*\" options=FtAggregateOptions { load: Named([\"data\", \"version\"]), paging: Cursor { count: 1500, max_idle: 30000 }, sort_by: [\"@sort_key\"] } all_args=[\"FT.AGGREGATE\", \"test:_content_prefix_sort_key_3e762c15\", \"*\", \"TIMEOUT\", \"10000\", \"LOAD\", \"2\", \"data\", \"version\", \"WITHCURSOR\", \"COUNT\", \"1500\", \"MAXIDLE\", \"30000\", \"SORTBY\", \"2\", \"@sort_key\", \"ASC\"]"
    ));
