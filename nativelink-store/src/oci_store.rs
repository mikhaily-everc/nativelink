...
impl OciStore {
    ...
    pub fn build_aws_spec(spec: &ExperimentalOciSpec) -> ExperimentalAwsSpec {
        ExperimentalAwsSpec {
            ...
            // Fork-only field (see `ExperimentalAwsSpec::single_upload_timeout_s`).
            // 0 selects the built-in default deadline, which is what upstream's
            // OCI store implicitly assumed before the field existed.
            single_upload_timeout_s: 0,
        }
    }
}
